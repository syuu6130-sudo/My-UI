...
local requestsDisabled = true --getgenv and getgenv().DISABLE_syu_u_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local syu_uFolder = "syu_u"
local ConfigurationFolder = syu_uFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		syu_uOpen = {Type = 'bind', Value = 'K', Name = 'syu_u Keybind'},
		-- buildwarnings
		-- syu_uprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.syu_uOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(syu_uFolder) then
				if isfile and isfile(syu_uFolder..'/settings'..ConfigurationExtension) then
					file = readfile(syu_uFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"syu_uOpen":{"Value":"K","Type":"bind","Name":"syu_u Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"syu_u Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('syu_u had an issue accessing configuration saving capability.')
		end
	end
end
...
local syu_u = useStudio and script.Parent:FindFirstChild('syu_u') or game:GetObjects("rbxassetid://10804731440")[1]
syu_u.Name = "syu_u"
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local syu_uDestroyed = false -- True when syu_uLibrary:Destroy() is called

repeat
	if syu_u:FindFirstChild('Build') and syu_u.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('syu_u | Build Mismatch')
		print('syu_u may encounter issues as you are running an incompatible interface version ('.. ((syu_u:FindFirstChild('Build') and syu_u.Build.Value) or 'No Build') ..').\n\nThis version of syu_u is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, syu_u = syu_u, useStudio and script.Parent:FindFirstChild('syu_u') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(syu_u)
	syu_u.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	syu_u.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == syu_u.Name and Interface ~= syu_u then
			Interface.Enabled = false
			Interface.Name = "syu_u-Old"
		end
	end
elseif not useStudio then
...
if syu_u.AbsoluteSize.X < minSize.X and syu_u.AbsoluteSize.Y < minSize.Y then
...
local Main = syu_u.Main
local MPrompt = syu_u:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = syu_u:FindFirstChild('Drag')
...
syu_u.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/syu_u/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = syu_u.Notifications

local SelectedTheme = syu_uLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = syu_uLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	syu_u.Main.BackgroundColor3 = SelectedTheme.Background
	syu_u.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	syu_u.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	syu_u.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	syu_u.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	syu_u.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	syu_u.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		syu_u.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		syu_u.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(syu_u:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("syu_u | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("syu_u | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('syu_u had an issue decoding the configuration file, please try delete the file and reopen syu_u.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(syu_uLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("syu_u | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--syu_uLibrary:Notify({Title = "syu_u Flags", Content = "syu_u was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(syu_uLibrary.Flags) do
...
function syu_uLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('syu_u | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			syu_uLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			syu_uLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "syu_uOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(syu_uFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('syu_u Settings', 0, true)

	if TabList['syu_u Settings'] then
		TabList['syu_u Settings'].LayoutOrder = 1000
	end

	if Elements['syu_u Settings'] then
		Elements['syu_u Settings'].LayoutOrder = 1000
	end
...
function syu_uLibrary:CreateWindow(Settings)
	print('creating window')
	if syu_u:FindFirstChild('Loading') then
		if getgenv and not getgenv().syu_uCached then
			syu_u.Enabled = true
			syu_u.Loading.Visible = true

			task.wait(1.4)
			syu_u.Loading.Visible = false
		end
	end

	if getgenv then getgenv().syu_uCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				syu_uLibrary:Notify({Title = 'Build Mismatch', Content = 'syu_u may encounter issues as you are running an incompatible interface version ('.. ((syu_u:FindFirstChild('Build') and syu_u.Build.Value) or 'No Build') ..').\n\nThis version of syu_u is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "syu_u"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "syu_u Interface Suite" then
		LoadingFrame.Version.Text = "syu_u UI"
	end
	...
	if not Settings.Disable_syu_uPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				syu_uLibrary:Notify({
					Title = "syu_u Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(syu_uFolder.."/Discord Invites") then
			makefolder(syu_uFolder.."/Discord Invites")
		end

		if isfile and not isfile(syu_uFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(syu_uFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"syu_u RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(syu_uFolder.."/Key System") then
			makefolder(syu_uFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("syu_u | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with syu_u specific development.')
				end
			end
		end
		...
		if isfile and isfile(syu_uFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(syu_uFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(syu_uFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						syu_uLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called syu_uLibrary:Destroy()
				if syu_uDestroyed then
					return
				end
				if not Success then
					...
					print("syu_u | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with syu_u specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= syu_uLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					syu_uLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			syu_u.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("syu_u | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with syu_u specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					syu_uLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("syu_u | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with syu_u specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					syu_uLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("syu_u | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with syu_u specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					syu_uLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= syu_uLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("syu_u | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with syu_u specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						syu_uLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("syu_u | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with syu_u specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					syu_uLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			syu_uLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			syu_uLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('syu_u had an issue creating settings.') end
...
function syu_uLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function syu_uLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function syu_uLibrary:Destroy()
	syu_uDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['syu_u Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "syu_uOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function syu_uLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				syu_uLibrary:Notify({Title = "syu_u Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			syu_uLibrary:Notify({Title = "syu_u Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('syu_u Configurations Error | '..tostring(result))
			syu_uLibrary:Notify({Title = "syu_u Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = syu_uLibrary:CreateWindow({
	--	Name = "syu_u Example Window",
	--	LoadingTitle = "syu_u Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using syu_u may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like syu_u to get the key from
	...
	----syu_uLibrary:Notify({Title = "syu_u Interface", Content = "Welcome to syu_u. These - are the brand new notification design for syu_u, with custom sizing and syu_u calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(syu_uLibrary.Theme) do
...
task.delay(4, function()
	syu_uLibrary.LoadConfiguration()
